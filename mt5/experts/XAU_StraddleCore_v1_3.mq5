...
//|                                                                  |
//|  ผลล่าสุด บัญชี $300 ล็อต 0.01 หักคอม+slippage แล้ว 10 seeds:      |
//|    นอกกลุ่มตัวอย่าง +19.4% (PF 1.37) | ข้อมูลสุ่ม -6.89 (ถูกต้อง)  |
//|    บวกทั้ง 4 สมมติฐานเส้นทางราคา 10/10 seeds                      |
//|                                                                  |
//|  *** ยังไม่ผ่านการยืนยันด้วย real tick data ***                   |
//|  *** ตัวอย่างมีแค่ 5 วัน - ทดสอบ Demo ก่อนเสมอ ***                |
//|                                                                  |
//|  ── ประวัติเวอร์ชัน ──────────────────────────────────────────   |
//|  1.3  จูนระยะคร่อม 0.50->0.30 และจุดกันทุน 0.60->0.40            |
//|       (กวาด 81 ชุด 12 อันดับแรกใช้ 0.30 ทุกชุด) ทำให้บวกทุก      |
//|       สมมติฐานเส้นทางราคา 10/10 seeds เป็นครั้งแรก               |
//|  1.2  เปลี่ยนช่วงเวลาเป็น 08:00-09:00 + 20:00-21:00 เลือกจาก     |
//|       โครงสร้างตลาด (ช่วงวิ่ง/สเปรด) ตัด 02:00-03:30 ทิ้ง         |
//|  1.1  แก้บั๊ก: ห้ามเชื่อ TimeGMT() ใน Strategy Tester             |
//|       (ทำให้ช่วงเวลาเลื่อนไป 4 ชม. โดยไม่มีอาการให้เห็น)          |
//|  1.0  เวอร์ชันแรก                                                 |
//+------------------------------------------------------------------+
...
//--- จูนรอบสุดท้าย 81 ชุด โดยตัดสินจาก "ความทนทาน" ไม่ใช่กำไรสูงสุด
//    (ต้องบวกนอกกลุ่มตัวอย่าง + ชนะข้อมูลสุ่ม + ไม่พังตอนสมมติว่าราคาหยาบ)
//    ผ่านเกณฑ์ 21 ชุด และ 12 อันดับแรกใช้ระยะคร่อม 0.30 ทุกชุดโดยไม่มีข้อยกเว้น
//    ระยะ 0.50 เดิมติดลบตอนสมมติฐานหยาบ (-39) ส่วน 0.30 เป็นบวก (+67)
//    ส่วน SL กับ InpLockGap ผลกระจัดกระจาย ไม่มีค่าไหนชนะชัด จึงใช้ค่ากลาง
//    เพื่อเลี่ยงการ fit กับข้อมูล 5 วัน
input double   InpEntryDist      = 0.30;   // ระยะคร่อมราคา (จูนแล้ว: 0.30)
input double   InpStopLoss       = 1.50;   // จุดตัดขาดทุน SL (ค่ากลาง ไม่ fit)
input double   InpTakeProfit     = 10.00;  // เป้ากำไร TP (0 = ไม่ตั้ง ให้ trailing พาออก)

input group "───────── 🔒 ล็อกกำไร ─────────"
input double   InpBEStart        = 0.40;   // กำไรถึงเท่าไร จึงเลื่อน SL มากันทุน (จูนแล้ว)
...
//--- ช่วงเวลาเลือกจาก "โครงสร้างตลาด" เป็นหลัก ไม่ใช่จากกำไรย้อนหลัง
//    วัดอัตราส่วน (ช่วงวิ่งเฉลี่ยต่อนาที / สเปรด) ของทุกชั่วโมง ซึ่งเป็น
//    คุณสมบัติที่นิ่งกว่าผลกำไรมาก  breakout จะคุ้มต้นทุนได้ก็ต่อเมื่อ
//    ราคาวิ่งมากพอ  สองชั่วโมงที่สูงสุดคือ:
//        ไทย 20:00  ratio 12.92  (วิ่ง $3.75/นาที)  <- ลอนดอน/นิวยอร์กทับกัน
//        ไทย 08:00  ratio 11.00  (วิ่ง $3.19/นาที)  <- โตเกียวเปิด
//    เทียบกับชั่วโมงที่แย่สุด ไทย 03:00 ratio เพียง 3.31
//
//    ช่วง 02:00-03:30 เดิมถูกตัดทิ้ง: ratio แค่ 5.69-7.33 และผลทดสอบติดลบ
//    ส่วนช่วงเย็นยาว 19:00-22:00 กำไรดีมากในกลุ่มตัวอย่าง (+296) แต่
//    ติดลบนอกกลุ่มตัวอย่าง (-23) = curve-fit จึงเหลือไว้เฉพาะชั่วโมงพีค
input string   InpSess1          = "08:00-09:00";  // ช่วงที่ 1 (โตเกียวเปิด)
input string   InpSess2          = "20:00-21:00";  // ช่วงที่ 2 (ลอนดอน/นิวยอร์กทับกัน)
input string   InpSess3          = "";             // ช่วงที่ 3 (ว่าง = ปิด)
